...
def data(name: str):
  try:
    ...
    res['pfCluster'] = clusterPF(res['data']['PF'], name)
    return res
  except Exception as e:
    print(e)
    abort(404)
...
def sampling():
  try:
    ...
    if not algoData:
      return {}
    indices = sampler(algoData, sampling_rate=sampling_rate)
    res = {}
...
if __name__ == '__main__':
  serve(server, host='0.0.0.0', port=5100)
